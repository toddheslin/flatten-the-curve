  ...
  cases_by_district.forEach(d => {
    ...
    d.cases.forEach(c => {
      if (d.structured_cases.hasOwnProperty(c.dateForSort)) {
        ...
        if (day.genders.hasOwnProperty(c.gender)) day.genders[c.gender] += 1
        else day.genders[c.gender == '' ? 'Undefined' : c.gender] = 1

        if (c.age != '') {
          if (day.ages.hasOwnProperty(c.age)) day.ages[c.age] += 1
          else day.ages[c.age] = 1
        }
      } else {
...
const createTimeSeries = cases_by_district => {
    ...
    _.forEach(d.structured_cases, day => {
      if (i == 0) {
        ...
        _.forEach(previous.ages, (val, key) => {
          if (new_day.ages.hasOwnProperty(key)) new_day.ages[key] += val
          else new_day.ages[key] = val
        })

        _.forEach(previous.genders, (val, key) => {
          if (new_day.genders.hasOwnProperty(key)) new_day.genders[key] += val
          else new_day.genders[key] = val
        })

        const since_last = differenceInCalendarDays(parseISO(new_day.dateObject), parseISO(previous.dateObject))
        if (since_last > 0) {
          ...
          for (let days_to_add = 1; days_to_add < since_last; days_to_add++) {
            ...
            additional_day.dateObject =
              format(addDays(decrementable_date_object, days_to_add), 'yyyy-MM-dd') + 'T00:00:00.000Z'
  ...
  cases_by_district.forEach(d => {
    _.forEach(d.time_series, day => {
      if (!dates.includes(day.dateForSort)) dates.push(day.dateForSort)
    })
  })
  dates = dates.sort()
  ...
  cases_by_district.forEach(d => {
    ...
    if (last_global_date != last_district_date) {
      ...
      let days_to_add = differenceInCalendarDays(parse(last_global_date, 'yyyyMMdd', new Date()), starting_date_object)
      while (days_to_add > 0) {
      ...
      d.time_series
        ...
        .forEach(d => {
          ...
          _.forEach(d.ages, (val, key) => {
            if (time_series_obj.ages.hasOwnProperty(key)) time_series_obj.ages[key] += val
            else time_series_obj.ages[key] = val
          })

          _.forEach(d.genders, (val, key) => {
            if (time_series_obj.genders.hasOwnProperty(key)) time_series_obj.genders[key] += val
            else time_series_obj.genders[key] = val
          })
        })
    })
      ...
      .then(response => {
        ...
        districts.forEach(name => {
          ...
          cases_by_district.push({
            name: name,
            cases: _.sortBy(
              cases.filter(c => c.district == name),
              'dateForSort'
            ),
          })
        })
